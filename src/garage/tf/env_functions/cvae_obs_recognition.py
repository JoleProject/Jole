...
class CVAEObsRecognition(EnvFunction2):
    """
    Continuous MLP QFunction.
    ...
    def _initialize(self):
        ...
        action_ph = tf.compat.v1.placeholder(tf.float32,
                                             ...
                                             name='act')
        next_obs_ph = tf.compat.v1.placeholder(tf.float32,
          (None, self._obs_dim), name="next_obs")

        with tf.compat.v1.variable_scope(self.name) as vs:
            self._variable_scope = vs
            self.model.build(obs_ph, action_ph, next_obs_ph)

        self._f_qval = tf.compat.v1.get_default_session().make_callable(
            self.model.networks['default'].outputs,
            feed_list=[obs_ph, action_ph, next_obs_ph])

    def get_obs_val(self, observation, action, next_obs):
        """Q Value of the network."""
        return self._f_qval(observation, action, next_obs)
    ...
    @overrides
    def get_fval_sym(self, state_input, action_input, next_obs_input, name):
        """
        ...
        with tf.compat.v1.variable_scope(self._variable_scope):
            return self.model.build(state_input, action_input, next_obs_input, name=name)
