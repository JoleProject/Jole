...
class CVAEObsGenerator(EnvFunction2):
    """
    Continuous MLP QFunction.
    ...
    def __init__(self,
        ...
        self.model = MLPMergeTwinModel(output_dim1=self._obs_dim,
                                   output_dim2=self._obs_dim,
    ...
    def _initialize(self):
        ...
        action_ph = tf.compat.v1.placeholder(tf.float32,
                                             ...
                                             name='act')
        z_var = tf.compat.v1.placeholder(tf.float32,
          (None, self._z_dim), name="z")

        with tf.compat.v1.variable_scope(self.name) as vs:
            self._variable_scope = vs
            self.model.build(obs_ph, action_ph, z_var)

        self._f_qval = tf.compat.v1.get_default_session().make_callable(
            self.model.networks['default'].outputs,
            feed_list=[obs_ph, action_ph, z_var])

    def get_obs_val(self, observation, action, z):
        """Q Value of the network."""
        return self._f_qval(observation, action, z)
    ...
    @overrides
    def get_fval_sym(self, state_input, action_input, z_input, name):
        """
        ...
        with tf.compat.v1.variable_scope(self._variable_scope):
            return self.model.build(state_input, action_input, z_input, name=name)
