...
class JoLeDDPG(OffPolicyRLAlgorithm):
        ...
        with tf.name_scope(self.name, 'JoLeDDPG'):
            ...
            with tf.name_scope('model_loss'):
                ...
                obs_model_loss = tf.reduce_mean(
                    tf.compat.v1.squared_difference(next_obs-obs, predicted_next_obs))
            ...
            sepe_predicted_next_obs = self.sepe_obs_model.get_fval_sym(obs, actions, name='sepe_obs_value')
            ...
            with tf.name_scope('model_loss'):
                ...
                sepe_obs_model_loss = tf.reduce_mean(
                    tf.compat.v1.squared_difference(next_obs-obs, sepe_predicted_next_obs))
    ...
    def train_once(self, itr, paths, obs_upper, obs_lower, action_upper, action_lower):
        ...
        for train_itr in range(self.n_train_steps):
            if self.replay_buffer.n_transitions_stored >= self.min_buffer_size:  # noqa: E501
                ...
                obs_model_loss, reward_model_loss, sepe_obs_model_loss, sepe_reward_model_loss = self.optimize_model(itr, paths)
    ...
    def optimize_model(self, itr, samples_data):
        ...
        if reward_model_loss > sepe_reward_model_loss:
            ...
            if self.sepe_reward_model_win >= 20:
    ...
    def optimize_jole(self, itr, samples_data, obs_upper, obs_lower, action_upper, action_lower, train_jole=True, jole_obs_action_type="observed"):
        transitions = self.replay_buffer.sample(self.buffer_batch_size)
        observations = transitions['observation']
        rewards = transitions['reward']
        actions = transitions['action']
        ...
        if train_jole:
            _, _, _, jole_loss = self.f_train_jole(jole_observations, jole_actions)
            #_, action_loss = self.f_train_policy(jole_observations)
        else:
            jole_loss = self.f_cal_jole_loss(jole_observations, jole_actions)
        return jole_loss
