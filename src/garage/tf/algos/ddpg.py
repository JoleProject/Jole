...
class DDPG(OffPolicyRLAlgorithm):
    ...
    def train_once(self, itr, paths, obs_upper, obs_lower, action_upper, action_lower, evaluate_paths):
        ...
        """
        #max_path_length = 0
        #max_length_done_stats = False
        #for path in paths:
        #    if path["running_length"] > max_path_length:
        #        max_path_length = path["running_length"]
        #        max_length_done_stats = path['dones'][-1]
        #print("num of paths:", len(paths))
        #print("max length status:", max_path_length, max_length_done_stats)
        ...
        self.episode_rewards.extend([
            path for path in evaluate_paths['undiscounted_returns']
        ])
        #self.episode_rewards.extend([
