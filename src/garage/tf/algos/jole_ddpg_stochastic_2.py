...
class JoLeDDPGStochastic(OffPolicyRLAlgorithm):
    ...
    def init_opt(self):
        ...
        with tf.name_scope(self.name, 'JoLeDDPG'):
            ...
            for i in range(self.num_z):
                regu_z = reguzs[i]
                jole_predicted_next_obs = self.obs_model_generator.get_fval_sym(jole_obs, jole_actions, regu_z, name="jole_obs_value{}"
                                                                                .format(i))
            ...
            obs_recontr_mean = self.obs_model_generator.get_fval_sym(obs, actions, z_obs_sample, "obs_model_generator")
            reconstr_loss_obs = tf.reduce_mean(tf.reduce_sum(tf.square(next_obs - obs_recontr_mean),1))
            ...
            reward_recontr_mean = self.reward_model_generator.get_fval_sym(obs, actions, z_reward_sample, "reward_model_generator")
            reconstr_loss_reward = tf.reduce_mean(tf.reduce_sum(tf.square(reward - reward_recontr_mean),1))
            ...
            sobs_recontr_mean = self.sepe_obs_model_generator.get_fval_sym(obs, actions, sz_obs_sample, "obs_model_generator")
            sreconstr_loss_obs = tf.reduce_mean(tf.reduce_sum(tf.square(next_obs - sobs_recontr_mean),1))
            ...
            sreward_recontr_mean = self.sepe_reward_model_generator.get_fval_sym(obs, actions, sz_reward_sample, "reward_model_generator")
            sreconstr_loss_reward = tf.reduce_mean(tf.reduce_sum(tf.square(reward - sreward_recontr_mean),1))
        ...
        for train_itr in range(self.n_train_steps):
            if self.replay_buffer.n_transitions_stored >= self.min_buffer_size:  # noqa: E501
                ...
                obs_model_loss, reward_model_loss, sepe_obs_model_loss, sepe_reward_model_loss,\
                    obs_model_reconstr_loss, reward_model_reconstr_loss, sepe_obs_model_reconstr_loss, sepe_reward_model_reconstr_loss = self.optimize_model(itr, paths, jole_obs, jole_actions, regu_zs)
    ...
    def optimize_model(self, itr, samples_data, jole_obs, jole_actions, regu_zs):
        ...
        _, obs_model_loss, obs_model_reconstr_loss = self.f_train_obs_model(next_observations, observations, actions, jole_obs, jole_actions, self.jole_clip_return_min, self.jole_clip_return_max, self.use_jole_obs, regu_zs, eps)
        ...
        return obs_model_loss, reward_model_loss, sepe_obs_model_loss, sepe_reward_model_loss, obs_model_reconstr_loss, reward_model_reconstr_loss, sepe_obs_model_reconstr_loss,sepe_reward_model_reconstr_loss
