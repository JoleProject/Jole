    ...
    def train(self, runner):
        ...
        for epoch in runner.step_epochs():
            ...
            for cycle in range(self.n_epoch_cycles):
                runner.step_path, obs_upper, obs_lower, action_upper, action_lower = runner.obtain_samples(runner.step_itr)
                evaluate_paths = runner.obtain_samples(runner.step_itr, is_evaluate=True)
                test_paths.extend(evaluate_paths)
                last_return = self.train_once(runner.step_itr,
                                              ...
                                              evaluate_paths)
                runner.step_itr += 1
            print("[evaluate info] num of evaluate paths:", len(test_paths))
            print("[evaluate info] max length of evaluate paths:", np.max([path["running_length"] for path in test_paths]))
            print("[evaluate info] average length of evaluate paths:", np.mean([path["running_length"] for path in test_paths]))
